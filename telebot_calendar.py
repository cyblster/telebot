...
def create_calendar(
    ...
    now_day = datetime.datetime.now(tz=TIMEZONE)
...
def create_months_calendar(
    ...
    if year is None:
        year = datetime.datetime.now(tz=TIMEZONE).year
    ...
    for i, month in enumerate(zip(MONTHS[0::2], MONTHS[1::2])):
        keyboard.add(
            InlineKeyboardButton(
                month[0], callback_data=calendar_callback.new("MONTH", year, i * 2 + 1, "!")
            ),
            InlineKeyboardButton(
                month[1],
                callback_data=calendar_callback.new("MONTH", year, i * 2 + 2, "!"),
            ),
        )
